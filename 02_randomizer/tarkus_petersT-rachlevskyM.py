# Team Tarkus -- Theodore Peters, Mai Rachlevsky
# SoftDev pd7
...
# Returns a random key from a dictionary <d>
def randKey(d):
    return choice(list(d))

# Prints a random member from the entry in a dictionary <d> corresponding to key <k>
def randMember(d, k):
    print(choice(d[k]))


key = randKey(KREWES)
randMember(KREWES, key)